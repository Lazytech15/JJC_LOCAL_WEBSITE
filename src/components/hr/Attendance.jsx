...
function Attendance() {
  ...
  try {
    ...
    const rangeStats = {
      ...
      total_regular_hours: attendanceResult.data.reduce((sum, r) => sum + (parseFloat(r.regular_hours) || 0), 0),
      total_overtime_hours: attendanceResult.data.reduce((sum, r) => sum + (parseFloat(r.overtime_hours) || 0), 0),
    ...
    summaryData.push([
      ...
      safeToFixed(avgHoursPerEmployee, 2) + "h",
    ...
    summaryData.push([
      "Punctuality Rate",
      safeToFixed(100 - latePercentage, 1) + "%",
    ...
    const totalHours = rangeStats.total_regular_hours + rangeStats.total_overtime_hours;
    const regularPercent = totalHours > 0 ? (rangeStats.total_regular_hours / totalHours) * 100 : 0;
    const overtimePercent = totalHours > 0 ? (rangeStats.total_overtime_hours / totalHours) * 100 : 0;
    
    summaryData.push([
      "Regular Hours", 
      "", 
      safeToFixed(rangeStats.total_regular_hours, 2),
      safeToFixed(regularPercent, 1) + "%"
    ]);
    summaryData.push([
      "Overtime Hours", 
      "", 
      safeToFixed(rangeStats.total_overtime_hours, 2),
      safeToFixed(overtimePercent, 1) + "%"
    ]);
    summaryData.push(["Late Arrivals", rangeStats.late_count, "", safeToFixed(latePercentage, 1) + "%"]);
    summaryData.push(["Clock Ins", rangeStats.clock_ins, "", ""]);
    summaryData.push(["Clock Outs", rangeStats.clock_outs, "", ""]);
    summaryData.push(["Unsynced Records", rangeStats.unsynced_count, "", safeToFixed(100 - syncPercentage, 1) + "%"]);
    summaryData.push([]); // Empty row
    ...
    if (Object.keys(rangeStats.department_breakdown).length > 0) {
      ...
      Object.entries(rangeStats.department_breakdown)
        .sort(([, a], [, b]) => b - a)
        .forEach(([dept, count]) => {
          const percentage = safeToFixed((count / rangeStats.total_records) * 100, 1);
          const employeesInDept = new Set(
            attendanceResult.data.filter(r => r.department === dept).map(r => r.employee_uid)
          ).size;
          const avgPerEmployee = employeesInDept > 0 ? safeToFixed(count / employeesInDept, 1) : "0";
    ...
    // Enhanced styling for summary sheet (styling code remains the same)
    ...
    attendanceResult.data.forEach((record) => {
      ...
      detailedData.push([
        ...
        safeToFixed(record.regular_hours, 2),
        safeToFixed(record.overtime_hours, 2),
    ...
    if (summaryResult.success && summaryResult.data.length > 0) {
      ...
      const generateRemarks = (summary) => {
        ...
        if (parseFloat(summary.total_hours || 0) >= 8 && !summary.has_late_entry && hasAnyClockIn && hasAnyClockOut) {
          return "";
        }
        ...
        if (parseFloat(summary.overtime_hours || 0) > 0 && (parseFloat(summary.regular_hours || 0) === 0)) {
          remarks.push("OT WITHOUT REG HOURS");
        }

        if (parseFloat(summary.overtime_hours || 0) > 4) {
          remarks.push("EXCESSIVE OT (" + safeToFixed(summary.overtime_hours, 1) + "h)");
        }

        if (parseFloat(summary.total_hours || 0) > 0 && parseFloat(summary.total_hours || 0) < 4) {
          remarks.push("MINIMAL HOURS (" + safeToFixed(summary.total_hours, 1) + "h)");
        }
        ...
        if (!hasAnyClockIn && !hasAnyClockOut && parseFloat(summary.total_hours || 0) === 0) {
      ...
      Object.keys(groupedByEmployee).sort().forEach((employeeKey, groupIndex) => {
        ...
        employeeRecords.forEach((summary) => {
          ...
          employeeSummaryData.push([
            ...
            safeToFixed(regularHours, 1),
            safeToFixed(overtimeHours, 1),
            sundayHours !== "" ? safeToFixed(sundayHours, 1) : "",
            remarks
          ]);
        });
        ...
        employeeSummaryData.push([
          "", "", "", "", "", "", "",
          `TOTAL HOURS`,
          safeToFixed(subtotalRegularHours, 1),
          safeToFixed(subtotalOvertimeHours, 1),
          subtotalSundayHours > 0 ? safeToFixed(subtotalSundayHours, 1) : "",
      ...
      employeeSummaryData.push([
        "", "", "", "", "", "", "",
        "GRAND TOTALS",
        safeToFixed(grandTotalRegularHours, 1),
        safeToFixed(grandTotalOvertimeHours, 1),
        grandTotalSundayHours > 0 ? safeToFixed(grandTotalSundayHours, 1) : "",
    ...
    if (summaryResult.success && summaryResult.data.length > 0) {
      summaryResult.data.forEach(summary => {
        ...
        perf.total_days++;
        perf.total_hours += parseFloat(summary.regular_hours || 0) + parseFloat(summary.overtime_hours || 0);
        if (summary.has_late_entry) perf.late_days++;
        if (summary.is_incomplete) perf.incomplete_days++;
        if (parseFloat(summary.overtime_hours || 0) > 0) perf.overtime_days++;
        ...
        if (!summary.has_late_entry && !summary.is_incomplete &&
          (parseFloat(summary.regular_hours || 0) + parseFloat(summary.overtime_hours || 0)) >= 8) {
          perf.perfect_days++;
        }
      });
      ...
      performanceArray.forEach(emp => {
        performanceData.push([
          ...
          emp.data.total_days,
          safeToFixed(emp.avgHours, 1),
          safeToFixed(emp.punctualityRate, 1) + "%",
          safeToFixed(emp.completionRate, 1) + "%",
          safeToFixed(emp.score, 1),
          emp.grade,
          emp.issues,
          emp.recommendations
        ]);
      ...
      Object.entries(gradeDistribution).forEach(([grade, count]) => {
        const percentage = safeToFixed((count / performanceArray.length) * 100, 1);
    ...
    attendanceResult.data.forEach(record => {
      ...
      if (parseFloat(record.overtime_hours || 0) > 0) {
    ...
    weekOrder.forEach(day => {
      if (dayOfWeekPattern[day]) {
        ...
        const latePercent = safeToFixed((data.late / data.total) * 100, 1);
        const otPercent = safeToFixed((data.overtime / data.total) * 100, 1);
    ...
    if (dayOfWeekPattern["Monday"] && dayOfWeekPattern["Monday"].late / dayOfWeekPattern["Monday"].total > 0.15) {
      insights.push({
        ...
        finding: `${safeToFixed((dayOfWeekPattern["Monday"].late / dayOfWeekPattern["Monday"].total) * 100, 1)}% late rate on Mondays`,
        impact: "Medium",
    ...
    if (weekendWork > rangeStats.total_records * 0.1) {
      insights.push({
        ...
        finding: `${weekendWork} weekend records (${safeToFixed((weekendWork / rangeStats.total_records) * 100, 1)}%)`,
        impact: "High",
    ...
    const fileName = `JJC_Attendance_Report_${filenameDatePart}_${new Date().getTime()}.xlsx`;
