        ...
        <CardContent className="p-8">
          <div className="flex flex-col items-center text-center mb-8">
            <Avatar className={`w-24 h-24 ring-4 mb-4 ${isDarkMode ? "ring-zinc-800" : "ring-zinc-200"}`}>
              <AvatarFallback
                className={`${isDarkMode ? "bg-zinc-800 text-white" : "bg-zinc-900 text-white"} text-3xl font-bold`}
              >
                {employee?.name
                  ?.split(" ")
                  .map((n) => n[0])
            ...
            <h3 className={`text-2xl font-bold ${isDarkMode ? "text-white" : "text-zinc-900"}`}>{employee?.name}</h3>
            <p className={`text-sm mt-1 ${isDarkMode ? "text-zinc-400" : "text-zinc-600"}`}>{employee?.position}</p>
          </div>
