...
// Ensure socket event system is available for real-time updates
// __DIR__ is .../api/routes/itemComponents, socket.php is at .../api/socket.php
...
function createPurchaseOrder() {
    ...
    $dbPriority = $priority;
        ...
        try {
            ...
            // ✅ Emit real-time event for PO creation
            // Use existing fields: id is the PO number; total_value is the field name in DB
            $socketEvents = getSocketEvents();
            $socketEvents->poCreated([
                'id' => $createdOrder['id'],
                'po_number' => $createdOrder['id'],
                'supplier' => $createdOrder['supplier'] ?? $createdOrder['supplier_name'] ?? null,
                'status' => $createdOrder['status'] ?? null,
                'total_amount' => $createdOrder['total_value'] ?? null,
                'order_date' => $createdOrder['order_date'] ?? $createdOrder['po_date'] ?? null,
            ]);
...
function updatePurchaseOrder($id)
{
    ...
    $deleteItemsStmt->execute([$poId]);
        ...
        // ✅ Emit real-time event for PO update
        $socketEvents = getSocketEvents();
        $socketEvents->poUpdated([
            'id' => $updatedOrder['id'],
            'po_number' => $updatedOrder['id'],
            'supplier' => $updatedOrder['supplier'] ?? $updatedOrder['supplier_name'] ?? null,
            'status' => $updatedOrder['status'] ?? null,
            'total_amount' => $updatedOrder['total_value'] ?? null,
        ]);
    ...
    try {
        ...
        if (!$order) {
            ...
            try {
                ...
                // Store PO number before deletion for event emission
                $deletedPONumber = $order['id'] ?? ($order['po_number'] ?? null);
                ...
                // ✅ Emit real-time event for PO deletion
                $socketEvents = getSocketEvents();
                $socketEvents->poDeleted([
                    'id' => $id,
