...
// Ensure socket event helpers are available for real-time updates
...
function handleStockUpdate($itemId) {
    ...
    // ✅ Emit real-time event for stock update (non-blocking)
    try {
        ...
        error_log('Socket event emission failed (stockUpdated): ' . $e->getMessage());
        // Do not interrupt the main response
    }
...
function handleStockInsert($itemId) {
    ...
    // ✅ Emit real-time event for stock insertion (non-blocking)
    try {
        $socketEvents = getSocketEvents();
        $socketEvents->stockInserted([
            'item_no' => $updatedItem['item_no'],
            'item_name' => $updatedItem['item_name'],
        ...
        error_log('Socket event emission failed (stockInserted): ' . $e->getMessage());
        // Do not interrupt the main response
    }
...
function handleStockQuantity($itemId) {
    ...
    $stmt = $db->prepare("UPDATE itemsdb SET in_qty = ?, out_qty = ? WHERE item_no = ?");
    $stmt->execute([$newInQty, $newOutQty, $itemId]);
    
    // Fetch updated item with recalculated values
    ...
    // ✅ Emit real-time event for stock quantity update (non-blocking)
    try {
        $socketEvents = getSocketEvents();
        $socketEvents->stockUpdated([
            'item_no' => $updatedItem['item_no'],
            ...
            'balance' => $updatedItem['balance'],
            'change_type' => $update_type ?: 'manual',
            'reason' => $notes,
...
$stmt->execute([$newOutQty, $itemId]);
    ...
    // ✅ Emit real-time event for stock removal (non-blocking)
    try {
        $socketEvents = getSocketEvents();
        $socketEvents->stockRemoved([
            'item_no' => $updatedItem['item_no'],
            'item_name' => $updatedItem['item_name'],
        ...
        error_log('Socket event emission failed (stockRemoved): ' . $e->getMessage());
        // Do not interrupt the main response
    }
