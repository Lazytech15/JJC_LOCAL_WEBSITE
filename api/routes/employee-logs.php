...
function createEmployeeLog() {
    ...
    // Use server time (NOW) instead of client-sent time for accuracy
    $log_date = date('Y-m-d'); // Current server date in YYYY-MM-DD format
    $log_time = date('H:i:s'); // Current server time in HH:MM:SS format
    
    // Prepare data object
    $logData = [];
    if ($username) $logData['username'] = $username;
    if ($details) $logData['details'] = $details;
    $logData['log_date'] = $log_date; // Always use server date
    $logData['log_time'] = $log_time; // Always use server time
    ...
    if ($items_json) $logData['items_json'] = $items_json;
    ...
    // ✅ Emit real-time event for employee log creation
    $socketEvents = getSocketEvents();
    $socketEvents->employeeLogCreated([
        'id' => $logEntry['id'],
        'username' => $logEntry['username'],
        'details' => $logEntry['details'],
        'log_date' => $logEntry['log_date'],
        'log_time' => $logEntry['log_time'],
...
function getAuditForLog($id) {
    ...
    try {
        ...
        // First, check what columns exist in the audit table
        $columnsQuery = $db->query("SHOW COLUMNS FROM employee_logs_audit");
        $columns = [];
        while ($col = $columnsQuery->fetch(PDO::FETCH_ASSOC)) {
            $columns[] = $col['Field'];
        }

        // Determine the correct timestamp column to order by
        $orderByColumn = 'id'; // Default fallback
        if (in_array('created_at', $columns)) {
            $orderByColumn = 'created_at';
        } elseif (in_array('audit_timestamp', $columns)) {
            $orderByColumn = 'audit_timestamp';
        } elseif (in_array('timestamp', $columns)) {
            $orderByColumn = 'timestamp';
        } elseif (in_array('audit_date', $columns)) {
            $orderByColumn = 'audit_date';
        }

        $stmt = $db->prepare("SELECT * FROM employee_logs_audit WHERE original_log_id = ? OR new_log_id = ? ORDER BY {$orderByColumn} DESC");
...
function updateEmployeeLog($id) {
    ...
    try {
        ...
        $allowedFields = ['username','details','log_date','log_time','purpose','id_number','id_barcode','item_no','items_json'];
        $changed = [];
        ...
        $auditColumns = ['original_log_id', 'new_log_id', 'admin_id', 'reason', 'changes_json', 'original_json', 'new_json'];
