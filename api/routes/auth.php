...
if ($endpoint === 'login' || $endpoint === null) {
    try {
        ...
        if ($method === 'POST') {
            ...
            $loginType = $input['loginType'] ?? null;
        } else {
            $username = $_GET['username'] ?? null;
            $password = $_GET['password'] ?? null;
            $department = $_GET['department'] ?? null;
            $loginType = $_GET['loginType'] ?? null; // NEW: Get loginType from query params
        }
        ...
        // ========================================================================
        // MODIFIED: Only check admin access_level if NOT employee login
        // ========================================================================
        if ($loginType !== 'employee' && $user['access_level'] !== 'admin') {
        ...
        if ($department === 'superAdmin') {
            ...
            // - Employee login: users can access their own department
            // - Admin login: only admins from that department can access
            if ($loginType === 'employee') {
                // Employee login: allow users to access their own department
                ...
                // Admin login: require admin access_level + department match (or SuperAdmin)
                if (!$isTrueSuperAdmin && $user['department'] !== $department) {
                    sendErrorResponse('User not authorized for this department. You can only access your assigned department: ' . $user['department'], 403);
                }
            }
        ...
        $userData = [
            ...
            'loginType' => $loginType // Include loginType in response for reference
        ];
